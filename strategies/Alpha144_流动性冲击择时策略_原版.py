...
# ── 数据要求 ──
# 注：已移除 MIN_HISTORY_BARS。khHistory 会自动向前拉取足够的历史数据，
#     无需等待 bar 积累。因子计算内部有数据长度检查，不足时自动跳过。
MIN_DAILY_AMOUNT = 3e7        # 最低日均成交额 (3000万, 过滤流动性极差股)

# ── 科创板过滤 ──
SKIP_KE_CHUANG_BAN = True    # 剔除科创板股票（688开头），波动大、止损触发频繁

# ── 入场质量过滤 ──
VOLUME_SURGE_RATIO = 0.8      # 成交量放大倍数（原版：0.8，即>>80%均量即可）
...
def init(stocks=None, data=None):
    ...
    logging.info("[Alpha144-原版] 初始化完成 | 刷新间隔=%d天 | 最大持仓=%d只 | 持有期=%d天 | 硬止损=%.0f%%" % (
...
def _buy_candidates(data, candidates, signals):
    """
    对突破候选现金等权买入。
    资金分配: 可用现金 / 剩余空位（避免 total_asset 膨胀导致仓位爆炸）
    """
