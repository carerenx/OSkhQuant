#coding:gbk
"""
Alpha#144 逆向选择持有策略 V5.4 — QMT原生版
====================================================
基于Alpha144因子的中盘股长期持有策略（OSkhQuant V5回测 +106.44%）。

V5.4 修复（终极）:
  - 用 xtdata.get_market_data_ex(dividend_type='front') 替换 get_history_data
  - xtdata 强制下载并应用复权因子，不依赖 QMT 本地缓存状态
  - 与 OSkhQuant khHistory(..., fq="pre") 行为一致

已知问题追踪:
  - V5.0: get_full_tick 返回成本价 → P&L 恒为0%
  - V5.1: 修复 P&L，但 get_history_data 默认不复权
  - V5.3: dividend_type=1 无效（本地缓存无复权因子，静默回退）
  - V5核心逻辑：纯MFE驱动退出 + 分段止损 + 分段止盈 + 大盘防御

核心理念:
  1. Alpha#144 = sum(|ret|/amount, ret<0, 20)
     因子值越小→下跌日流动性冲击越小→筹码最稳定→适合长期持有
  2. 逆向选择 — 每30天选Alpha144最低的12只，等权持有
  3. 纯MFE驱动退出 — MFE与终盈相关性+0.872，是最强预测指标
     - MFE<5% + 持有>30天 → 清仓（核心）
  4. 水下时间止损（连续水下>80天）
  5. 僵尸仓清退（>80天+MFE<5%）
  6. 分段止损（<50d:-25% / >=50d:-15%）
  7. 分段移动止盈（MFE>30%→5%紧线 / 否则10%标准线）
  8. 大盘两级防御（MA60软防御/硬防御）
  9. 剔除创业板(300)/科创板(688)
...
class State:
    ...
    bar_counter = 0
    last_barpos = -1
    positions = {}         # {code: {shares, entry_price, entry_bar, bars_held, highest_price, underwater_days}}
    pending_sells = []
    acc_id = 'testS'
    capital = 500000       # 初始资金 50万
    cash = 0
    total_assets = 0
    need_bars = 0
...
def init(ContextInfo):
    print("[Alpha144-V5] 纯MFE驱动退出 QMT版")
    ...
    print("[init] V5 初始化完成 | 持仓=%d只 | MFE:%dd<%.0f%% | 水下=%dd | 僵尸>%dd+MFE<%.0f%% | 分段止损 | 分段止盈" % (
...
def handlebar(ContextInfo):
    ...
    # ===== V5.4: xtdata 强制前复权 (dividend_type='front') =====
    # get_history_data 的 dividend_type=1 依赖本地缓存中的复权因子，
    # 若无缓存则静默回退为不复权。xtdata 会主动下载复权数据。
    ...
    # ═══════════════════════════════════════════════════════════
    # 2. MFE疲劳退出（V5核心） + 水下止损 + 僵尸仓 + 分段止损 + 分段止盈
    # ═══════════════════════════════════════════════════════════
...
def _check_underwater_stop(ContextInfo, hist_close):
    ...
    for code, pos in State.positions.items():
        ...
        if px <= 0: continue
        pnl_pct = (px / entry - 1.0) * 100
        print("  [水下止损] %s 连续水下%d天 >= %d天 -> 清仓 | entry=%.2f now=%.2f pnl=%.1f%%" % (
...
def _check_zombie(ContextInfo, hist_close):
    ...
    for code, pos in State.positions.items():
        ...
        if mfe < ZOMBIE_MFE_THRESHOLD:
            ...
            if px <= 0: continue
            pnl_pct = (px / entry - 1.0) * 100
            print("  [僵尸仓] %s 持有%d天 MFE仅%.1f%% < %.0f%% -> 清仓 | entry=%.2f now=%.2f pnl=%.1f%%" % (
...
def _sell_removed_stocks(ContextInfo, hist_close):
    ...
    for code in list(State.positions.keys()):
        ...
        if pos.get('bars_held', 0) < PROTECTION_BARS:
            print("  [调仓保护] %s 持有仅%d天 < %d天保护期，暂不卖出" % (code, pos.get('bars_held', 0), PROTECTION_BARS))
            continue
...
def _get_price(ContextInfo, code, hist_close):
    # V5.1 修复: 优先使用历史收盘价（回测中 get_full_tick 可能返回成本价导致 P&L=0）
