...
核心理念:
  ...
  2. 逆向选择 — 每30天选Alpha144最低的12只股票，等权持有
  3. 多层MFE疲劳退出（20d/<3%, 30d/<5%, 40d/<8%）
  4. 分段止损（<50d:-25% / ≥50d:-15%）+ 分段移动止盈（5%/10%双线）
  5. 组合回撤止损（60日峰值回落>25%→清仓）
  6. 月度动量防御（连续2月亏损+大盘下行→暂停买入）
...
# ── 多层MFE疲劳退出 ──
MFE_TIERS = [
    {'bars': 20, 'threshold': 0.03, 'name': 'MFE-L1(20d<3%)'},
    {'bars': 30, 'threshold': 0.05, 'name': 'MFE-L2(30d<5%)'},
    {'bars': 40, 'threshold': 0.08, 'name': 'MFE-L3(40d<8%)'},
]
...
PORTFOLIO_DD_THRESHOLD = 0.25

# ── 月度动量过滤 ──
MOMENTUM_WINDOW = 2
MOMENTUM_MA_WINDOW = 20
...
def init(ContextInfo):
    print("[Alpha144-V6] 多层MFE+组合风控 QMT版")
    ...
    print("[init] V6 初始化完成 | 持仓=%d只 | MFE:20d<3%%,30d<5%%,40d<8%% | 组合DD>25%%清仓 | 动量防御%d月" % (
        BOTTOM_N, MOMENTUM_WINDOW))
...
def handlebar(ContextInfo):
    ...
    # ── V6: 月度动量追踪 ──
    _update_monthly_momentum(ContextInfo)
    ...
    # 3. 月度动量防御
    # ═══════════════════════════════════════════════════════════
    momentum_defense = _check_momentum_defense(hist_close)
    ...
    # 8. 调仓 + 买入
    # ═══════════════════════════════════════════════════════════
    if market_state == 'ok' and State.bottom_n and not momentum_defense:
        _sell_removed_stocks(ContextInfo, hist_close)
        _buy_new_stocks(ContextInfo)
    elif market_state == 'soft_defense':
        print("[Alpha144-V6] 软防御：暂停买入/调仓")
    elif momentum_defense:
        print("[Alpha144-V6] 动量防御：暂停买入，持仓继续管理")
    ...
    print("[摘要] 持仓=%d只 %s | 动量=%s | 下次刷新=%d" % (
        len(State.positions),
        pos_codes[:5] if pos_codes else "空仓",
        "防御" if momentum_defense else "正常",
...
def _compute_factor_rankings(hist_close, hist_amount):
    ...
    for code in State.stock_pool:
        ...
        try:
            recent_amt = np.array(amount_arr[-FACTOR_WINDOW:], dtype=float)
            if np.mean(recent_amt) < MIN_DAILY_AMOUNT: continue
        except Exception: continue
        ann_vol = _calc_annual_vol(close_arr)
        if ann_vol > MAX_ANNUAL_VOL: continue
    ...
    for code, pos in State.positions.items():
        ...
        for tier in MFE_TIERS:
            if bars >= tier['bars'] and mfe < tier['threshold']:
                ...
                print("  [MFE疲劳] %s %s | entry=%.2f now=%.2f pnl=%.1f%%" % (
                    code, tier['name'], entry, px, pnl_pct))
...
def _update_monthly_momentum(ContextInfo):
    try:
        t = ContextInfo.get_bar_timetag(ContextInfo.barpos)
        month = timetag_to_datetime(t, '%Y-%m')
    except Exception:
        return
    ...
    if not State.current_month:
        ...
        State.month_start_asset = ta
        return

    if month != State.current_month:
        monthly_ret = (ta / State.month_start_asset - 1.0) if State.month_start_asset > 0 else 0
        State.monthly_returns.append((State.current_month, monthly_ret))
        if len(State.monthly_returns) > 6:
            State.monthly_returns = State.monthly_returns[-6:]
        consecutive = 0
        for _, ret in reversed(State.monthly_returns):
            if ret < 0: consecutive += 1
            else: break
        State.consecutive_loss_months = consecutive
        State.current_month = month
        State.month_start_asset = ta


def _check_momentum_defense(hist_close):
    if State.consecutive_loss_months < MOMENTUM_WINDOW:
        return False

    if BENCHMARK not in hist_close:
        return State.consecutive_loss_months >= MOMENTUM_WINDOW

    arr = hist_close[BENCHMARK]
    if len(arr) < MOMENTUM_MA_WINDOW + 1:
        return State.consecutive_loss_months >= MOMENTUM_WINDOW
    ...
    try:
        ...
        if current < ma:
            print("  [动量防御] 连续%d月亏损+大盘%.2f<MA%d=%.2f → 暂停买入" % (
                State.consecutive_loss_months, current, MOMENTUM_MA_WINDOW, ma))
            return True
    except Exception:
        pass
    return State.consecutive_loss_months >= MOMENTUM_WINDOW
...
def _sell_position(ContextInfo, code, hist_close, reason):
    ...
    if shares <= 0:
        del State.positions[code]; return
...
def _get_fallback_csi500():
    return [
        '002001.SZ','002007.SZ','002008.SZ','002013.SZ','002019.SZ',
        '002020.SZ','002022.SZ','002025.SZ','002049.SZ','002050.SZ',
        '002064.SZ','002065.SZ','002074.SZ','002080.SZ','002085.SZ',
        '002092.SZ','002097.SZ','002108.SZ','002120.SZ','002121.SZ',
        '002129.SZ','002138.SZ','002155.SZ','002156.SZ','002185.SZ',
        '002195.SZ','002202.SZ','002203.SZ','002230.SZ','002240.SZ',
        '002245.SZ','002250.SZ','002258.SZ','002261.SZ','002273.SZ',
        '002281.SZ','002304.SZ','002326.SZ','002340.SZ','002352.SZ',
        '002368.SZ','002371.SZ','002373.SZ','002384.SZ','002405.SZ',
        '002407.SZ','002408.SZ','002409.SZ','002410.SZ','002436.SZ',
        '002440.SZ','002456.SZ','002459.SZ','002460.SZ','002463.SZ',
        '002465.SZ','002468.SZ','002475.SZ','002532.SZ','002555.SZ',
        '002558.SZ','002602.SZ','002624.SZ','002625.SZ','002673.SZ',
        '002714.SZ','002738.SZ','002739.SZ','002797.SZ','002966.SZ',
        '000021.SZ','000034.SZ','000060.SZ','000066.SZ','000400.SZ',
        '000401.SZ','000423.SZ','000425.SZ','000426.SZ','000528.SZ',
        '000537.SZ','000538.SZ','000547.SZ','000553.SZ','000559.SZ',
        '000581.SZ','000623.SZ','000625.SZ','000629.SZ','000630.SZ',
        '000636.SZ','000656.SZ','000661.SZ','000683.SZ','000703.SZ',
        '000708.SZ','000709.SZ','000723.SZ','000728.SZ','000729.SZ',
        '000733.SZ','000737.SZ','000738.SZ','000739.SZ','000750.SZ',
        '000776.SZ','000778.SZ','000783.SZ','000786.SZ','000800.SZ',
        '000807.SZ','000825.SZ','000826.SZ','000830.SZ','000831.SZ',
        '000860.SZ','000869.SZ','000878.SZ','000883.SZ','000887.SZ',
