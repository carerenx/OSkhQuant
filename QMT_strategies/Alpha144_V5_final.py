#coding:gbk
"""
Alpha#144 逆向选择持有策略 V5 — QMT最终版
====================================================
基于Alpha144因子的中盘股长期持有策略。

核心理念:
  1. Alpha#144 = sum(|ret|/amount, ret<0, 20)
     因子值越小 → 下跌日流动性冲击越小 → 筹码稳定 → 适合长期持有
  2. 逆向选择 — 每30天选Alpha144最低的12只，等权持有
  3. 纯MFE驱动退出 — MFE与终盈相关性+0.872
  4. 分段止损 + 分段止盈 + 大盘两级防御

关键修复（对比原始V5.0）:
  - get_history_data 添加 dividend_type=1（前复权），与OSkhQuant fq="pre" 一致
  - _get_price 优先使用 hist_close（QMT回测中 get_full_tick 返回成本价导致P&L=0）

⚠️ 运行前必须:
  1. QMT菜单 → 数据管理 → 清空本地数据缓存 → 确定
  2. QMT菜单 → 数据管理 → 下载历史数据 → 日线/中证500/2020-至今 → 下载
  3. QMT菜单 → 设置 → K线复权方式 → 前复权
  (不清空缓存的话前复权无法生效，价格会是错的)

回测参数:
  - 初始资金: 50万 | 基准: 000905.SH | 回测: 2024-01-01 ~ 至今
...
# ── MFE疲劳退出（核心：MFE<5%+持有>30天→清仓） ──
MFE_FATIGUE_BARS      = 30
MFE_FATIGUE_THRESHOLD  = 0.05

# ── 水下时间止损 ──
UNDERWATER_STOP_DAYS = 80

# ── 僵尸仓清退 ──
ZOMBIE_BARS           = 80
ZOMBIE_MFE_THRESHOLD  = 0.05
...
def init(ContextInfo):
    ...
    for method_name, method_fn in [
        ...
        try:
            ...
            if raw and len(raw) > 0:
                stocks = raw
                print("[init] %s 获取到 %d 只" % (method_name, len(raw)))
                break
        except Exception:
            continue
    ...
    print("[init] 初始化完成 | 持仓=%d只 | MFE:%dd<%.0f%% | 分段止损/止盈 | 大盘MA%d防御" % (
        BOTTOM_N, MFE_FATIGUE_BARS, MFE_FATIGUE_THRESHOLD*100, MA_MARKET))
    print("[init] 回测: 2024-01 ~ 至今 | 初始资金: %.0f万" % (State.capital / 10000))
...
def handlebar(ContextInfo):
    ...
    # ===== xtdata.get_market_data_ex(dividend_type='front') 前复权 =====
    cur_date = timetag_to_datetime(ContextInfo.get_bar_timetag(bar), '%Y%m%d')
    start_dt = datetime.strptime(cur_date, '%Y%m%d') - timedelta(days=need_bars * 2)
    start_date = start_dt.strftime('%Y%m%d')

    all_syms = State.stock_pool[:] + [BENCHMARK]
    for sym in all_syms:
        xtdata.download_history_data(sym, '1d', start_date, cur_date)

    raw_close = xtdata.get_market_data_ex(
        field_list=['close'], stock_list=all_syms, period='1d',
        start_time=start_date, end_time=cur_date, dividend_type='front')
    raw_amount = xtdata.get_market_data_ex(
        field_list=['amount'], stock_list=all_syms, period='1d',
        start_time=start_date, end_time=cur_date, dividend_type='front')

    hist_close  = {s: [] for s in all_syms}
    hist_amount = {s: [] for s in all_syms}
    for sym in all_syms:
        if sym in raw_close and raw_close[sym] is not None and len(raw_close[sym]) > 0:
            hist_close[sym] = [float(x) for x in raw_close[sym].iloc[:, 0].values]
        if sym in raw_amount and raw_amount[sym] is not None and len(raw_amount[sym]) > 0:
            hist_amount[sym] = [float(x) for x in raw_amount[sym].iloc[:, 0].values]
    ...
    # 2. MFE疲劳 + 水下止损 + 僵尸仓 + 分段止损 + 分段止盈
    _check_mfe_fatigue(ContextInfo, hist_close)
    _check_underwater_stop(ContextInfo, hist_close)
    _check_zombie(ContextInfo, hist_close)
    ...
    if market_state == 'ok' and State.bottom_n:
        ...
        _buy_new_stocks(ContextInfo, hist_close)
    elif market_state == 'soft_defense':
        print("[Alpha144-V5] 软防御：暂停买入/调仓")
...
def _check_mfe_fatigue(ContextInfo, hist_close):
    ...
    for code, pos in State.positions.items():
        ...
        if mfe < MFE_FATIGUE_THRESHOLD:
            ...
            print("  [MFE疲劳] %s 持有%d天 MFE仅%.1f%% < %.0f%% | entry=%.2f now=%.2f pnl=%.1f%%" % (
...
def _check_underwater_stop(ContextInfo, hist_close):
    to_sell = []
    for code, pos in State.positions.items():
        uw_days = pos.get('underwater_days', 0)
        if uw_days < UNDERWATER_STOP_DAYS: continue
        entry = pos.get('entry_price', 0)
        if entry <= 0: continue
        px = _get_price(ContextInfo, code, hist_close)
        if px <= 0: continue
        pnl_pct = (px / entry - 1.0) * 100
        print("  [水下止损] %s 连续水下%d天 >= %d天 | entry=%.2f now=%.2f pnl=%.1f%%" % (
            code, uw_days, UNDERWATER_STOP_DAYS, entry, px, pnl_pct))
...
def _check_zombie(ContextInfo, hist_close):
    to_sell = []
    for code, pos in State.positions.items():
        bars = pos.get('bars_held', 0)
        if bars < ZOMBIE_BARS: continue
        entry = pos.get('entry_price', 0)
        if entry <= 0: continue
        highest = pos.get('highest_price', entry)
        mfe = (highest / entry - 1.0)
        if mfe < ZOMBIE_MFE_THRESHOLD:
            ...
            if px <= 0: continue
            pnl_pct = (px / entry - 1.0) * 100
            print("  [僵尸仓] %s 持有%d天 MFE仅%.1f%% < %.0f%% | entry=%.2f now=%.2f pnl=%.1f%%" % (
                code, bars, mfe * 100, ZOMBIE_MFE_THRESHOLD * 100, entry, px, pnl_pct))
...
def _sell_removed_stocks(ContextInfo, hist_close):
    ...
    for code in list(State.positions.keys()):
        ...
        if pos.get('bars_held', 0) < PROTECTION_BARS:
            print("  [调仓保护] %s 持有仅%d天 < %d天保护期" % (code, pos.get('bars_held', 0), PROTECTION_BARS))
            continue
...
def _buy_new_stocks(ContextInfo, hist_close):
    if not State.bottom_n: return
    ...
    for code in to_buy:
        ...
        State.positions[code] = {
            ...
            'highest_price': px, 'underwater_days': 0,
        }
...
def _get_price(ContextInfo, code, hist_close):
    """
    获取当前价格。
    优先使用 hist_close 最新收盘价（回测中更可靠），tick 数据作为补充（实盘）。
    """
        ...
        for p in ps:
            ...
            if code in State.positions:
                ...
                remote[code] = {
                    ...
                    'entry_bar': State.last_barpos, 'bars_held': 0,
                    'highest_price': p.m_dOpenPrice, 'underwater_days': 0,
                }
        for code in list(State.positions.keys()):
...
def _get_fallback_csi500():
    return [
        ...
        '600004.SH','600007.SH','600008.SH','600021.SH','600026.SH','600029.SH',
        '600032.SH','600079.SH','600085.SH','600096.SH','600109.SH','600126.SH',
        '600132.SH','600143.SH','600157.SH','600161.SH','600166.SH','600196.SH',
        '600208.SH','600256.SH','600276.SH','600282.SH','600316.SH','600348.SH',
        '600350.SH','600361.SH','600363.SH','600369.SH','600377.SH','600378.SH',
        '600380.SH','600390.SH','600418.SH','600486.SH','600487.SH','600497.SH',
        '600498.SH','600499.SH','600511.SH','600521.SH','600536.SH','600546.SH',
        '600563.SH','600566.SH','600578.SH','600580.SH','600642.SH','600655.SH',
        '600673.SH','600688.SH','600699.SH','600711.SH','600720.SH','600724.SH',
        '600739.SH','600754.SH','600763.SH','600764.SH','600801.SH','600808.SH',
        '600816.SH','600820.SH','600839.SH','600863.SH','600871.SH','600873.SH',
        '600879.SH','600884.SH','600895.SH','600906.SH','600909.SH','600985.SH',
        '600988.SH','600995.SH','601016.SH','601019.SH','601098.SH','601099.SH',
        '601106.SH','601108.SH','601118.SH','601128.SH','601139.SH','601156.SH',
        '601162.SH','601168.SH','601179.SH','601198.SH','601212.SH','601233.SH',
        '601236.SH','601456.SH','601555.SH','601577.SH','601665.SH','601666.SH',
        '601696.SH','601717.SH','601928.SH','601990.SH','601991.SH','601997.SH',
        '603000.SH','603049.SH','603077.SH','603087.SH','603160.SH','603175.SH',
        '603444.SH','603501.SH','603529.SH','603565.SH','603568.SH','603606.SH',
        '603658.SH','603707.SH','603728.SH','603786.SH','603816.SH','603826.SH',
        '603833.SH','603858.SH','603868.SH','603885.SH','603893.SH','603939.SH',
        '603986.SH',
    ]
